...
edgeWorkerProxyServer.use('/', httpProxy(`http://localhost:${process.env.ESI_PROXY_PORT}`, {
    userResDecorator: async (proxyRes, proxyResData, userReq, userRes) => {
        try {
            ...
            for ( const jsBundle of jsBundles ) {
                preloadModuleLinks.push(`<link rel="modulepreload" href="${jsBundle}" as="script">`)
            }

            for ( const cssBundle of cssBundles ) {
                preloadLinks.push(`<link rel="preload" href="${cssBundle}" as="style">`)
